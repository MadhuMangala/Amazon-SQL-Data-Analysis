...
select month(ord.order_date) as"month" ,year(ord.order_date) as "year",
...
```sql
	...
	#==========================================================
	declare product_flag int;
	declare inventory_flag int;
	declare max_order_id int;
	declare prd_price float;
	declare prd_id_check int;
	#==========================================================
	...
	#==========================================================
	select 1 into inventory_flag from inventory
	where inventory.product_id = prd_id_check
		and inventory.stock >= in_quantity;
	#==========================================================
	# get the max order_id for new order entry into ords table
	#==========================================================
	select max(order_id) into max_order_id from order_items;
	#==========================================================
	# check the prd and inventory qty and update the info
	#==========================================================
	if inventory_flag > 0 and product_flag > 0 then
	# order_items table updations
	...
	# update the invetory 
		update inventory
		 ...
		 where inventory.product_id = prd_id_check;
	# updated information flag
	select "Thanks for purchasing the product !"as flag,in_product_name;
	#===========================================================
	else
		select "Product Quantity is out of stock!" as flag,in_quantity;
	end if;
